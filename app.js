document.addEventListener('DOMContentLoaded', () => {
    ...
    function selectDive(id) {
        ...
        document.getElementById('mind-summary-text').innerText = `Entspannungseffekt: ${log.stressPre - log.stressPost} Punkte.`;
    ...
    function updateDashboard() {
        ...
        if (diveLogs.length > 0) {
            ...
            if (flowPostEl) flowPostEl.innerText = avgPost;
            if (flowIndexEl) flowIndexEl.innerText = avgFlow;
        }
    }
    ...
    window.mockPokedexSearch = async function() {
        const query = document.getElementById('pokedex-search').value;
        if (!query) return;
        const resDiv = document.getElementById('pokedex-result');
        resDiv.style.display = 'block';
        resDiv.innerHTML = '<p style="color: var(--clr-accent);">🔍 Suche...</p>';
        try {
            const resp = await fetch('/api/scan', { method: 'POST', body: JSON.stringify({ query }), headers: { 'Content-Type': 'application/json' } });
            const data = await resp.json();
            renderPokedexResult(data, resDiv);
        } catch (err) { resDiv.innerHTML = `<p style="color: var(--clr-warning);">⚠️ Fehler: ${err.message}</p>`; }
    }
